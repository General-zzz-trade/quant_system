...
pub fn rust_allocate_portfolio<'py>(
    ...
    for (i, sym) in symbols.iter().enumerate() {
        ...
        let w = weights[i];
        let notional_abs = w.abs() * equity;
        let qty = (w * equity) / px;
        notionals.push(notional_abs);
        qtys.push(qty);
    }
    ...
    if let Some(tc) = turnover_cap {
        if tc > 0.0 {
            if let Some(cq) = current_qty {
                ...
                let mut cur_notionals: Vec<f64> = Vec::with_capacity(symbols.len());
                for i in 0..symbols.len() {
                    let px: f64 = prices
                        .get_item(&symbols[i])?
                        .map(|v| v.extract::<f64>().unwrap_or(0.0))

...
features/        Feature computation (185+ features, V1-V24)
  ...
  rust/              15 .rs (FeatureEngine, indicators, incremental trackers)

risk/            Risk gates (StagedRisk, AdaptiveStop, GateChain)
  rust/              6 .rs (gate_chain, risk engine, adaptive stop)
...
monitoring/      Ops (watchdog, IC decay, data quality, Telegram)
...
Bybit WS kline → MarketEvent → EngineCoordinator.emit()
  ├─ FeatureComputeHook → RustFeatureEngine → 120+ features
...
**Trading & safety**:
- `MAX_ORDER_NOTIONAL_PCT = 150%` of equity (safety cap); dynamic via `get_max_order_notional(equity)`

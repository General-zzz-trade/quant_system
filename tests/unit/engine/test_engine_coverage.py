...
class TestSpan:
    def test_finish_sets_end_ns(self):
        from engine.tracing import Span
        s = Span(span_id="abc", name="test", start_ns=0)
    ...
    def test_attrs_default_empty(self):
        from engine.tracing import Span
        s = Span(span_id="x", name="y", start_ns=0)
        assert s.attrs == {}
...
class TestTrace:
    def test_add_span(self):
        from engine.tracing import Trace, Span
        tr = Trace(trace_id="tid")
        s = Span(span_id="sid", name="root", start_ns=0)
        tr.add_span(s)
        assert "sid" in tr.spans
        assert tr.spans["sid"] is s
...
class TestTracer:
    ...
    def test_current_trace_id_set_after_start_trace(self):
        from engine.tracing import Tracer
        t = Tracer()
        tid = t.start_trace("root")
        assert t.current_trace_id() == tid

    def test_snapshot_returns_trace(self):
        from engine.tracing import Tracer
        t = Tracer()
        tid = t.start_trace("root")
        snap = t.snapshot(tid)
    ...
    def test_start_span_within_trace(self):
        ...
        t = Tracer()
        t.start_trace("root")
        span = t.start_span("child", attrs={"k": "v"})
        assert span.name == "child"
        assert span.attrs == {"k": "v"}
        assert t.current_span_id() == span.span_id

    def test_start_span_without_trace_auto_creates_trace(self):
        from engine.tracing import Tracer
        t = Tracer()
        # No trace started — should auto-create
        span = t.start_span("orphan")
        assert span is not None
        assert t.current_trace_id() is not None

    def test_end_span_reverts_to_parent(self):
        from engine.tracing import Tracer
        t = Tracer()
        t.start_trace("root")
        root_span_id = t.current_span_id()
        child = t.start_span("child")
        assert t.current_span_id() == child.span_id
        t.end_span(child)
        assert t.current_span_id() == root_span_id
    ...
    def test_end_trace_clears_ids(self):
        from engine.tracing import Tracer
        t = Tracer()
        t.start_trace("root")
        t.end_trace()
    ...
    def test_start_trace_with_attrs(self):
        from engine.tracing import Tracer
        t = Tracer()
        tid = t.start_trace("root", attrs={"symbol": "ETHUSDT"})
        snap = t.snapshot(tid)
        # The root span should have our attrs
        root_span = list(snap.spans.values())[0]
        assert root_span.attrs.get("symbol") == "ETHUSDT"

    def test_current_ids_none_before_start(self):
        from engine.tracing import Tracer
        t = Tracer()
        assert t.current_trace_id() is None
        assert t.current_span_id() is None

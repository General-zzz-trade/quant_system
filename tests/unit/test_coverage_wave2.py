...
# ══════════════════════════════════════════════════════════════════════════════
# Section 8: execution/routing/smart_router.py
# ══════════════════════════════════════════════════════════════════════════════

from execution.routing.smart_router import (  # noqa: E402
    VenueQuote, RouteDecision, SmartRouter,
)


class TestSmartRouter:
    def _quote(self, venue_id, bid, ask, bid_qty=100, ask_qty=100, latency=1.0):
        return VenueQuote(
            venue_id=venue_id,
            bid=Decimal(str(bid)),
            ask=Decimal(str(ask)),
    ...
    def test_register_and_list_venues(self):
        router = SmartRouter()
        p = MagicMock()
        router.register_venue("bybit", p)
        assert "bybit" in router.venue_ids

    def test_unregister_venue(self):
        router = SmartRouter()
        p = MagicMock()
        router.register_venue("binance", p)
        router.unregister_venue("binance")
        assert "binance" not in router.venue_ids

    def test_collect_quotes_empty(self):
        router = SmartRouter()
        assert router.collect_quotes("BTCUSDT") == []

    def test_collect_quotes_with_provider(self):
        router = SmartRouter()
        p = MagicMock()
        q = self._quote("binance", 50000, 50001)
        p.get_quote.return_value = q
        router.register_venue("binance", p)
        quotes = router.collect_quotes("BTCUSDT")
        assert len(quotes) == 1

    def test_collect_quotes_skips_high_latency(self):
        router = SmartRouter(max_latency_ms=100.0)
        p = MagicMock()
        q = self._quote("binance", 50000, 50001, latency=200.0)
        p.get_quote.return_value = q
        router.register_venue("binance", p)
        quotes = router.collect_quotes("BTCUSDT")
        assert len(quotes) == 0

    def test_collect_quotes_handles_exception(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.side_effect = RuntimeError("connection failed")
        router.register_venue("binance", p)
        quotes = router.collect_quotes("BTCUSDT")
        assert len(quotes) == 0

    def test_collect_quotes_skips_none(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = None
        router.register_venue("binance", p)
        quotes = router.collect_quotes("BTCUSDT")
        assert len(quotes) == 0

    def test_route_order_no_venues(self):
        router = SmartRouter()
        assert router.route_order("BTCUSDT", "buy", Decimal("1")) is None

    def test_route_order_buy_best_ask(self):
        router = SmartRouter()
        for venue_id, ask in [("bybit", "50001"), ("binance", "50000")]:
            p = MagicMock()
            p.get_quote.return_value = self._quote(venue_id, 49999, ask)
            router.register_venue(venue_id, p)

        decision = router.route_order("BTCUSDT", "buy", Decimal("10"))
        assert decision.venue_id == "binance"
        assert decision.price == Decimal("50000")
        assert decision.reason == "best_ask"

    def test_route_order_sell_best_bid(self):
        router = SmartRouter()
        for venue_id, bid in [("bybit", "49999"), ("binance", "50000")]:
            p = MagicMock()
            p.get_quote.return_value = self._quote(venue_id, bid, "50001")
            router.register_venue(venue_id, p)

        decision = router.route_order("BTCUSDT", "sell", Decimal("10"))
        assert decision.venue_id == "binance"
        assert decision.price == Decimal("50000")
        assert decision.reason == "best_bid"

    def test_route_order_buy_fallback_when_insufficient_qty(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 49999, 50000, ask_qty=5)
        router.register_venue("binance", p)
    ...
    def test_route_order_sell_fallback_when_insufficient_qty(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 50000, 50001, bid_qty=5)
        router.register_venue("binance", p)

        decision = router.route_order("BTCUSDT", "sell", Decimal("10"))
        assert decision is not None

    def test_route_order_no_valid_asks(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 49999, 0, ask_qty=100)
        router.register_venue("binance", p)
        decision = router.route_order("BTCUSDT", "buy", Decimal("1"))
        assert decision is None

    def test_route_order_no_valid_bids(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 0, 50001, bid_qty=100)
        router.register_venue("binance", p)
        decision = router.route_order("BTCUSDT", "sell", Decimal("1"))
        assert decision is None

    def test_split_order_no_venues(self):
        router = SmartRouter()
        assert router.split_order("BTCUSDT", "buy", Decimal("10")) is None

    def test_split_order_buy(self):
        router = SmartRouter()
        for i, ask in enumerate([50000, 50001, 50002]):
            p = MagicMock()
            p.get_quote.return_value = self._quote(f"v{i}", 49999, ask, ask_qty=5)
            router.register_venue(f"v{i}", p)

        split = router.split_order("BTCUSDT", "buy", Decimal("12"))
        assert split is not None
        assert len(split.legs) >= 2
        assert split.total_qty == Decimal("12")

    def test_split_order_sell(self):
        router = SmartRouter()
        for i, bid in enumerate([50000, 49999]):
            p = MagicMock()
            p.get_quote.return_value = self._quote(f"v{i}", bid, 50001, bid_qty=5)
            router.register_venue(f"v{i}", p)

        split = router.split_order("BTCUSDT", "sell", Decimal("10"))
        assert split is not None
        assert split.total_qty == Decimal("10")
        assert len(split.legs) == 2

    def test_split_order_partial_fill(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 50000, 50001, ask_qty=5)
        router.register_venue("binance", p)

        split = router.split_order("BTCUSDT", "buy", Decimal("10"))
        assert split is not None
        assert split.total_qty == Decimal("5")

    def test_split_order_no_valid_quotes(self):
        router = SmartRouter()
        p = MagicMock()
        p.get_quote.return_value = self._quote("binance", 0, 0)
        router.register_venue("binance", p)
        split = router.split_order("BTCUSDT", "buy", Decimal("10"))
        assert split is None

    def test_venue_quote_fields(self):
        q = VenueQuote(
            venue_id="test", bid=Decimal("100"), ask=Decimal("101"),
            bid_qty=Decimal("10"), ask_qty=Decimal("10"), latency_ms=5.0,
        )
        assert q.venue_id == "test"
        assert q.latency_ms == 5.0

    def test_route_decision_fields(self):
        d = RouteDecision(
            venue_id="binance", price=Decimal("50000"),
            available_qty=Decimal("10"), reason="best_ask",
        )
        assert d.reason == "best_ask"

...
**Crate stats**: 68 Rust source files under `ext/rust/src/`, 64 PyO3 classes + 106 functions exported via `_quant_hotpath`, standalone `quant_trader` binary included.
...
| `ext/rust/` | 68 | Rust crate, PyO3 kernel, and standalone `quant_trader` binary |
| `features/` | 27 | Feature computation, enriched computer, cross-asset |
| `decision/` | 89 | DecisionEngine, signals, sizing, execution policy, regime/rebalance modules |
...
| `execution/` | 204 | Exchange adapters, order state machine, dedup, reconciliation, algos |
| `portfolio/` | 80 | Black-Litterman, Kelly, risk parity, risk model, optimizers |
...
| `runner/` | 28 | LiveRunner, PaperRunner, BacktestRunner, graceful shutdown |
| `monitoring/` | 27 | Prometheus metrics, Grafana dashboards, Telegram alerts, health server |
| `infra/` | 23 | Config loader, structured JSON logging, model signing, threading utils |
| `scripts/` | 101 | Training, walk-forward validation, alpha research, data download |
| `research/` | 23 | Experiment tracking, Monte Carlo, sensitivity analysis, model registry |
| `tests/` | 284 | Unit + integration + replay + regression + performance tests |
...
quant_system/
  ...
  ext/rust/          # Rust crate + PyO3 kernel + standalone binary (68 Rust files)
  features/          # Feature engineering (27 files)
  decision/          # Trading decision logic (89 files)
  alpha/             # ML alpha models and inference (30 files)
  risk/              # Risk management and kill switch (19 files)
  execution/         # Exchange connectivity and execution platform (204 files)
  ...
  runner/            # Entry points: live, paper, backtest (28 files)
  monitoring/        # Prometheus, Grafana, Telegram (27 files)
  infra/             # Config, logging, secrets (23 files)
  scripts/           # Training, research, data download (101 files)
  research/          # Experiment tracking, Monte Carlo (23 files)
  _quant_hotpath/    # Built Rust shared library
  tests/             # Unit + integration + replay + regression + performance (284 files)
